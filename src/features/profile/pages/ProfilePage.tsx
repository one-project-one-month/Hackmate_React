...
import { Mail, User, Github, Pencil, Loader2 } from "lucide-react";
import TechStackBadge from "../components/TechStackBadge";
import ProfileStat from "../components/ProfileStat";
import { useAppDispatch, useAppSelector } from "@/hooks/useAppHook";
import { fetchUserProfile } from "../api";
import { setProfileData, setProfileError, setProfileLoading } from "../slice";
...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const loadProfile = async () => {
      // Avoid double fetching if we already have data
      if (data) return;
      ...
      try {
        ...
        dispatch(setProfileError(err.message || "Failed to fetch profile"));
  ...
  if (isLoading && !data) {
    return (
      <div className="min-h-screen py-5 px-4 flex justify-center items-center">
        <Loader2 className="animate-spin text-cyan-400 w-10 h-10" />
      </div>
    );
  }

  const profileImage = data?.profile_image || "https://api.dicebear.com/7.x/avataaars/svg?seed=Hsu";
  const name = data?.name || data?.username || "Unknown User";
  const email = data?.email || "No email provided";
  const role = data?.preferred_role || "Not specified";
  const techStacks = data?.tech_stacks || [];
  const stats = data?.stats || { completed: 0, inProgress: 0, kick: 0 };
    ...
    <div className="min-h-screen py-5 px-4 md:px-8 lg:px-16 flex flex-col justify-start items-start">
      ...
      <div className="relative flex flex-col py-8 px-6 md:py-10 md:px-10 lg:py-12 lg:px-16 w-full md:max-w-4xl lg:max-w-full min-h-[500px] bg-white/10 backdrop-blur-lg rounded-3xl border border-white/5">
        <button 
          ...
          className="cursor-pointer absolute top-4 right-4 md:top-6 md:right-6 lg:top-8 lg:right-8 text-zinc-400 hover:text-white transition-colors"
        >
          <Pencil size={20} />
        </button>

        {error && (
          <div className="w-full text-center text-red-500 mb-4 bg-red-500/10 p-2 rounded-lg">
            {error}
          </div>
        )}
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 lg:gap-8 max-w-2xl pr-12 mb-10 text-center md:text-left">
          ...
          <div className="flex flex-col gap-2 lg:gap-3 items-center md:items-start">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 md:gap-4 lg:gap-6 text-zinc-300">
              <span className="flex items-center gap-2 text-xs lg:text-sm">
                <User size={14} className="lg:w-4 lg:h-4 text-cyan-400" /> {role}
              </span>
              <span className="flex items-center gap-2 text-xs lg:text-sm">
                <Mail size={14} className="lg:w-4 lg:h-4 text-cyan-400" /> {email}
              </span>
            </div>
            <button className="cursor-pointer mt-2 flex items-center justify-center gap-2 px-3 py-1.5 lg:px-4 border border-zinc-500 rounded-lg text-xs lg:text-sm text-zinc-200 hover:bg-white/5 transition-all w-fit">
        ...
        <div className="flex justify-between items-center w-full px-2 md:px-4 lg:px-10 mb-12 text-center">
          ...
          <ProfileStat label="In Progress" value={stats.inProgress} />
          <div className="h-10 lg:h-16 w-[3px] bg-zinc-200 mx-2 rounded-full"></div>
          <ProfileStat label="Kick" value={stats.kick} />
        </div>
        ...
        <div className="w-full mt-auto">
          ...
          {techStacks.length > 0 ? (
            ...
            <div className="text-sm text-zinc-400 italic">No tech stacks added yet.</div>
          )}
        </div>
      </div>
    </div>
